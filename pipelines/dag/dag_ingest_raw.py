...
N_SEDES = 3  # sede_0, sede_1, sede_2
...
EXTERNAL_FILES = [
    ("financial_data.csv", "sources/financial/", "financial"),
    ("healthcare_dataset.csv", "sources/financial/", "financial"),
    ("inventory_data.csv", "sources/operational/", "operational"),
]
...
def upload_parquet_with_checksum(
    ...
    key_prefix = f"{RAW_PREFIX}source={source}/sede=sede_{sede}/date={date_str}/"
...
def read_csv_from_s3(s3_hook: S3Hook, key: str) -> pd.DataFrame:
...
def ingest_dataframe_partitioned_by_sede(
    ...
    """Asigna sede por fila (hash(sede_col) % N_SEDES), particiona el df
    en N_SEDES sub-dataframes y escribe cada uno como parquet independiente.

    Si la columna sede_col no existe, se usa el índice del DataFrame como
    fallback para poder simular sede en datasets externos.
    """
    if sede_col in df.columns:
        key_series = df[sede_col]
    else:
        logger.warning(
            "Columna '%s' no encontrada en %s; usando índice de fila para asignar sede",
            sede_col,
            base_filename,
        )
        key_series = df.index
    ...
    df = df.copy()
    df["_sede_simulada"] = key_series.map(assign_sede)

    results = []
    for sede in range(N_SEDES):
        sub_df = df[df["_sede_simulada"] == sede].drop(columns=["_sede_simulada"])
        if sub_df.empty:
            continue
...
def dag_ingest_raw():
    ...
    def ingest_mimic(verificacion: dict, **context) -> list[dict]:
        ...
        for filename in MIMIC_FILES:
            ...
            df = read_csv_from_s3(s3_hook, key)
    ...
    def registrar_metadata_ingesta(
        ...
        **context,
        ...
        """Escribe el log de metadata de la ingesta como JSON en
        raw/_metadata/ingestion_log/, ya que en el lab la trazabilidad
        se guarda en S3."""
        s3_hook = S3Hook(aws_conn_id=AWS_CONN_ID)
